   ...
   <header>
    ...
    <main class="main">
        ...
        <div class="screen">
            <div class="article">
                ...
                <?php while($row = $stmt->fetch(PDO::FETCH_ASSOC)){ ?>
                    <a href="empires-art.php?id=<?= $row["id"]?>"><img src="<?= htmlspecialchars($row["img"])?>" alt=""></a>
                    <h1><a href="empires-art.php?id=<?= $row["id"]?>"><?= htmlspecialchars($row['nom'])?></a></h1>
                <?php } ?>
            </div>
            ...
            <div class="articles">
                <?php
                    $db = ConnectionDB();
                    $stmt = $db->query("SELECT * FROM empires ORDER BY id DESC LIMIT 1, 5");
                ?>
                <?php while($row = $stmt->fetch(PDO::FETCH_ASSOC)){ ?>
                    <div class="espace" style="display: flex;">
                        <img src="./styles/flèche.svg" alt="" height="30px" width="30px">
                        <p><a href="empires-art.php?id=<?= $row["id"]?>"><?= htmlspecialchars($row['nom'])?></a></p>
                    </div>
                <?php } ?>
            </div>
        </div>        
        ...
        <div class="article-empires">
            ...
            <?php
                $db = ConnectionDB();
                $stmt = $db->query("SELECT * FROM empires ORDER BY id DESC LIMIT 5, 100")
            ?>
            <?php while($row = $stmt->fetch(PDO::FETCH_ASSOC)){?>
